...
% xs      = P.StartingPoint();		 % x0 is some starting point
% [gs,fs] = F.oracle(xs);
[g0,f0] = F.oracle(x0); a0 = 1;
P.InitialCondition( (x0-xs)^2 + a0* (f0-fs-1/2/param.L*g0^2)<= 1); % Initial condition ||x0-xs||^2<= 1
...
[double(fN-fs)  1/2/theta{N+1}^2]  % worst-case objective function accuracy
